...
while not build_button:
    ...
    approve_button = button(grey, 1625, y_deg, 275, 50, "Approve Put Down")
    y_deg += 75

    build_button = True

# declaration of the may I buttons

may_i_button = []

may_i_button.append(button(grey, 850, 125, 25, 25, "1"))
may_i_button.append(button(grey, 900, 125, 25, 25, "2"))
may_i_button.append(button(grey, 950, 125, 25, 25, "3"))

new_may_i_button = button(grey, 875, 60, 75, 50, "May I")  # literal button you press to activate it

# end declaration of the may I buttons
...
validate_card_down_button.append(button(grey, 1775, 700, 125, 30, "Approve"))
validate_card_down_button.append(button(grey, 1775, 745, 125, 30, "Approve"))
validate_card_down_button.append(button(grey, 1775, 790, 125, 30, "Approve"))

confirm_put_down_button = button((255, 0, 0), 1625, 835, 110, 30, "Confirm")

cancel_put_down_button = button(grey, 1775, 835, 110, 30, "Cancel")

# end declaration of approve put down buttons


player_x_coords = [[25, 75, 125], [550, 600, 650], [1075, 1125, 1175]]
player_y_coords = [[300, 500, 700], [300, 500, 700], [300, 500, 700]]

# print(player_x_coords[0])
# print(confirm_put_down)
...
#     # set_pile.append(discard_current)
...
# def build_player_1_put_down():
#     # global player_hand
#     # build_set.append(player_hand[player][0])
#     # player_hand[player].pop(0)
#     #
#     # build_set.append(player_hand[player][1])
#     # player_hand[player].pop(1)
#     #
#     # build_set.append(player_hand[player][2])
#     # player_hand[player].pop(2)
#     #
#     # lop = 0
#     # # x_deg = 100        for showing all cards
#     # x_deg = -25  # for showing only one card
#     # while lop < len(build_set):
#     #     discard_current = build_set[lop]
#     #     # disc_current = pygame.image.load(
#     #     # r'H:\Python\images\cards/' + discard_current.value + discard_current.suit_type[0] + '.png').convert()
#     #
#     #     set_new.append(pygame.image.load(
#     #         r'H:\Python\images\cards/' + discard_current.value + discard_current.suit_type[0] + '.png').convert_alpha())
#     #
#     #     set_new[lop] = pygame.transform.scale(set_new[lop], (80, 140)).convert_alpha()
#     #     # Scale the loaded card image
#     #     lop += 1
#
#     global disc
...
def display_set_player2():
    ...
    while lop < len(player2_hand_put_down):
        x_deg += 50  # for showing all cards
        screen.blit(player2_hand_put_down[lop], (x_deg, 300))
        lop += 1
    ...
    while lop < len(player2_hand_put_down):
        x_deg += 50  # for showing all cards
        screen.blit(player2_hand_put_down[lop], (x_deg, 500))
        lop += 1
    ...
    while lop < len(player2_hand_put_down):
        x_deg += 50  # for showing all cards
        screen.blit(player2_hand_put_down[lop], (x_deg, 700))
        lop += 1
...
def display_set_player3():
    ...
    while lop < len(player3_hand_put_down):
        x_deg += 50  # for showing all cards
        screen.blit(player3_hand_put_down[lop], (x_deg, 300))
        lop += 1
    ...
    while lop < len(player3_hand_put_down):
        x_deg += 50  # for showing all cards
        screen.blit(player3_hand_put_down[lop], (x_deg, 500))
        lop += 1
    ...
    while lop < len(player3_hand_put_down):
        x_deg += 50  # for showing all cards
        screen.blit(player3_hand_put_down[lop], (x_deg, 700))
        lop += 1


# def build_display_set1():
#     global set_pile
#     lop = 0
#     # x_deg = 100        for showing all cards
#     x_deg = -25  # for showing only one card
#     while lop < len(player1_put_down[put_down_array_counter]):
#         #discard_current = set_pile[lop]
#
#         discard_current = player1_put_down[put_down_array_counter][lop]
#         # disc_current = pygame.image.load(
#         # r'H:\Python\images\cards/' + discard_current.value + discard_current.suit_type[0] + '.png').convert()
#
...
#             r'H:\Python\images\cards/' + discard_current.value + discard_current.suit_type[0] + '.png').convert_alpha())
#
#         player1_hand_put_down[put_down_array_counter][lop] = pygame.transform.scale(player1_hand_put_down[put_down_array_counter][lop], (80, 140)).convert_alpha()
#         # Scale the loaded card image
#         lop += 1
...
def display_set():
    lop = 0
    x_deg = -25  # for showing only one card
    while lop < len(player1_hand_put_down):
        x_deg += 50  # for showing all cards
        screen.blit(player1_hand_put_down[lop], (x_deg, 300))
        lop += 1
...
def display_set2():
    ...
    while lop < len(player1_hand_put_down):
        x_deg += 50  # for showing all cards
        screen.blit(player1_hand_put_down[lop], (x_deg, 500))
        lop += 1
...
def display_set3():
    ...
    while lop < len(player1_hand_put_down):
        x_deg += 50  # for showing all cards
        screen.blit(player1_hand_put_down[lop], (x_deg, 700))
        lop += 1
...
# player_put_down = [player1_put_down, player2_put_down, player3_put_down]

# player1_put_down[0] = 12
# player_put_down[1] = 12

# print(player1_put_down)
...
while not notOver:
    ...
    while True:
        ...
        for ev in pygame.event.get():
            ...
            if ev.type == pygame.MOUSEBUTTONDOWN:
                ...
                if validate_card_down_button[2].isOver(pos) or put_down_button[2].text == "Null":
                    ...
                    # if validate_card_down_button[0].color == green:
                    #     confirm_put_down[0] = True
                    #
                    # if validate_card_down_button[1].color == green:
                    #     confirm_put_down[1] = True
                    #
                    # if put_down_button[2].text == "Null" or validate_card_down_button[2] == green:
                    #     confirm_put_down[2] = True
        ...
        approve_button.draw(screen, (0, 0, 0))
